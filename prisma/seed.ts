...
async function main() {
  ...
  const flightEntities = await prisma.flight.findMany({
    take: 5,
  });

  for (const flight of flightEntities) {
    // Create a booking
    const booking = await prisma.booking.create({
      data: {
        userId: user.id,
        totalAmount: flight.economyPrice * 2, // Assume 2 passengers
        status: 'CONFIRMED',
        paymentStatus: 'PAID',
      },
    });
    ...
    await prisma.flightSegment.create({
      data: {
        ...
        flightId: flight.id,
        cabinClass: 'ECONOMY',
        fareAmount: flight.economyPrice,
        isReturn: false,
      },
    });
  }
